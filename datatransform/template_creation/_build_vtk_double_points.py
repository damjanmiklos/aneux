"""Compile vtk_double_points against the active env's VTK (hemomesh on this machine)."""
...
def extension():
    ...
    if not os.path.isfile(os.path.join(vtk_inc, "vtkPoints.h")):
        raise FileNotFoundError(f"vtkPoints.h not found under {vtk_inc}")
    compile_args = []
    if os.name == "nt":
    ...
    return Extension(
        "vtk_double_points",
        sources=[os.path.join(HERE, "vtk_double_points.cpp")],
...
def build_inplace():
    ext = extension()
    dist = Distribution({"name": "vtk_double_points", "ext_modules": [ext]})
    ...
    cmd.run()
    print("built", os.path.join(HERE, cmd.get_ext_filename("vtk_double_points")))
