...
def clipVoronoiDiagram(directory, filename, aneur_no):
    ...
    def MaskWithPatch(id, t, c, r, maskArray, centerlines, voronoi):
        ...
        for i in range(voronoi.GetNumberOfPoints()):
            ...
            if (spherevalue < 0.0) & (voronoiVectorDot < 0.0):
                continue
            elif (tubevalue <= 0.0):
    ...
    def ExtractMaskedVoronoiPoints(voronoi, maskArray, increase):
        ...
        for i in range(voronoi.GetNumberOfPoints()):
            point = [0.0, 0.0, 0.0]
            voronoi.GetPoint(i, point)
            # TODO: arbitrary increase w/o much reasoning
            pointRadius = voronoi.GetPointData().GetArray(radiusArrayName).GetTuple1(i) * increase
    ...
    # OPTIONS TO SET
    smoothVoronoiDiagram = 0  # recommended; perform smoothing of the Voronoi Diagram to
    ##				remove smaller maximal spheres
    smoothingFactor = 0.05  # usually set to  0.25; the bigger the smoothing factor the
    ##				smaller the number of removed spheres.

    voronoiDiagramFilename = os.path.join(directory, filename + '_' + str(aneur_no) + '_voronoi.vtp')
    ...
    # outputfilenames:
    clippedVoronoiDiagramFilename = os.path.join(directory, filename + '_' + str(aneur_no) + '_clippedvoronoi.vtp')
    smoothVoronoiDiagramFilename = os.path.join(directory, filename + '_' + str(aneur_no) + '_smoothclippedvoronoi.vtp')
    ...
    aneurysmCenterline = ReadPolyData(aneurysmCenterlineFilename)
    ...
    clippedVoronoiDiagram = ExtractMaskedVoronoiPoints(originalVoronoiDiagram, maskedVoronoiArray,
                                                       1.0)  # 1.2 set to remove artifacts
    ...
    print('Clipping Aneurysm Voronoi Diagram')
    maskedVoronoiArray = MaskVoronoiDiagram(originalVoronoiDiagram, aneurysmCenterline)
    clippedVoronoiDiagram = ExtractMaskedVoronoiPoints(originalVoronoiDiagram, maskedVoronoiArray, 1.0)
    WritePolyData(clippedVoronoiDiagram, aneurysmVoronoiDiagramFilename)
