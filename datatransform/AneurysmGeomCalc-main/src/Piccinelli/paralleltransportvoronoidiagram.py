...
def parallelTransportVoronoiDiagram(directory, filename, aneur_no):
    ...
    def VoronoiDiagramInterpolation(interpolationcellid, id0, id1, voronoiDataset0, voronoiDataset1, centerlines,
                                    direction):
    ...
    def ExtractLine(cellid, centerlines):
        ...
        for i in range(numberOfPoints):
            ...
            normal = centerlines.GetPointData().GetArray(parallelTransportNormalsArrayName).GetTuple3(
                cell.GetPointId(i))
    ...
    # OPTIONS TO SET
    interpolationHalfSize = 3 #3
    polyBallImageSize = [100, 100, 100]  #[90, 90, 90]  size of the image for the evaluation of the polyball function


    smoothedVoronoi = 0

    interpolatedCenterlinesFilename = os.path.join(directory, filename + '_' + str(aneur_no) + '_interpolatedcl.vtp')
    patchCenterlinesFilename = os.path.join(directory, filename + '_' + str(aneur_no) + '_patchcl.vtp')
    if smoothedVoronoi == 1:
        clippedVoronoiFilename = os.path.join(directory, filename + '_' + str(aneur_no) + '_smoothclippedvoronoi.vtp')
    else:
        clippedVoronoiFilename = os.path.join(directory, filename + '_' + str(aneur_no) + '_clippedvoronoi.vtp')
    clippingPointsFilename = os.path.join(directory, filename + '_' + str(aneur_no) + '_clippingpoints.vtp')

    aneurysmVoronoiDiagramFilename = os.path.join(directory, filename + '_' + str(aneur_no) + '_aneurysmvoronoi.vtp')

    # outputfilenames:
    completeVoronoiFilename = os.path.join(directory, filename + '_' + str(aneur_no) + '_completevoronoi.vtp')
    surfaceFilename = os.path.join(directory, filename + '_' + str(aneur_no) + '_reconstructedmodel.vtp')

    aneurysmSurfaceFilename = os.path.join(directory, filename + '_' + str(aneur_no) + '_aneurysmsurface.vtp')
    interpolatedVoronoiFilename = os.path.join(directory,
                                               filename + '_' + str(aneur_no) + '_parentartery.vtp')
    ...
    aneurysmVoronoi = ReadPolyData(aneurysmVoronoiDiagramFilename)

    completeVoronoiDiagram = vtk.vtkPolyData()
    completeVoronoiDiagram.DeepCopy(clippedVoronoi)

    interpolatedVoronoiDiagram = vtk.vtkPolyData()

    aneurysmVoronoiDiagram = vtk.vtkPolyData()
    aneurysmVoronoiDiagram.DeepCopy(aneurysmVoronoi)
    ...
    for j in range(1, numberOfInterpolatedCenterlinesCells + 1):
        ...
        startCellPointHalfRadius = startCellPointRadius / 7.0 #7.0

        startInterpolationDataset = ExtractCylindricInterpolationVoronoiDiagram(startId, startCellPointId,
                                                                                startCellPointRadius, clippedVoronoi,
                                                                                patchCenterlines)
        startHalfInterpolationDataset = ExtractCylindricInterpolationVoronoiDiagram(startId, startCellPointId,
        ...
        endInterpolationDataset = ExtractCylindricInterpolationVoronoiDiagram(endId, endCellPointId, endCellPointRadius,
                                                                              clippedVoronoi, patchCenterlines)
        endHalfInterpolationDataset = ExtractCylindricInterpolationVoronoiDiagram(endId, endCellPointId,
                                                                                  endCellPointHalfRadius,
                                                                                  clippedVoronoi,
                                                                                  patchCenterlines)

        newVoronoiPoints, newVoronoiPointsMISR = VoronoiDiagramInterpolation(interpolationCellId, startId, endId,
        ...
        interpolatedVoronoiDiagram = InsertNewVoronoiPoints(interpolatedVoronoiDiagram, newVoronoiPoints,
                                                            newVoronoiPointsMISR)

        newVoronoiPoints, newVoronoiPointsMISR = VoronoiDiagramInterpolation(interpolationCellId, endId, startId,
                                                                             endInterpolationDataset,
                                                                             startHalfInterpolationDataset,
                                                                             interpolatedCenterlines, 0)
        completeVoronoiDiagram = InsertNewVoronoiPoints(completeVoronoiDiagram, newVoronoiPoints, newVoronoiPointsMISR)
        interpolatedVoronoiDiagram = InsertNewVoronoiPoints(interpolatedVoronoiDiagram, newVoronoiPoints,
                                                            newVoronoiPointsMISR)

    WritePolyData(interpolatedVoronoiDiagram, interpolatedVoronoiFilename)
    ...
    print('Reconstructing Aneurysm Surface from Voronoi Diagram')
    modeller = vtkvmtk.vtkvmtkPolyBallModeller()
    modeller.SetInputData(aneurysmVoronoiDiagram)
    modeller.SetRadiusArrayName(radiusArrayName)
    modeller.UsePolyBallLineOff()
    modeller.SetSampleDimensions(polyBallImageSize)
    modeller.Update()
    ...
    marchingCube = vtk.vtkMarchingCubes()
    marchingCube.SetInputData(modeller.GetOutput())
    marchingCube.SetValue(0, 0.0) #0
    marchingCube.Update()
    envelope = marchingCube.GetOutput()
    WritePolyData(envelope, aneurysmSurfaceFilename)
