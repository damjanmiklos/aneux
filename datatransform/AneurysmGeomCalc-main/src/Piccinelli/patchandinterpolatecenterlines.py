...
def patchandInterpolateCenterlines(directory, filename, aneurysmType, aneur_no):
    ...
    def SaveParentArtery(centerlines):
        ...
        # Ez a rész szedi le az aneurizmát!
        count = 0
        for i in range(1, numberOfCells):  # cell0 is the one that goes to the aneurysm dome
            ...
            for j in range(cell.GetNumberOfPoints()):
                ...
                radiusArray.SetTuple1(count,
                                      centerlines.GetPointData().GetArray(radiusArrayName).GetTuple1(
                                          cell.GetPointId(j)))
    ...
    def SaveAneurysm(centerlines):
        numberOfCells = centerlines.GetNumberOfCells()

        cell0 = centerlines.GetCell(1)
        numberOfArteryPoints = cell0.GetNumberOfPoints()
        ...
        # Ez a rész menti az aneurizma centerlinet
        count = 0
        cell = vtk.vtkGenericCell()
        centerlines.GetCell(0, cell)
        ...
        for j in range(cell.GetNumberOfPoints()):
            arteryPoints.InsertNextPoint(cell.GetPoints().GetPoint(j))
            radiusArray.SetTuple1(count,
                                  centerlines.GetPointData().GetArray(radiusArrayName).GetTuple1(cell.GetPointId(j)))
            arteryCellArray.InsertCellPoint(count)
            count += 1
    ...
    def FindClippingPointOnParentArtery(centerlines, parentCenterlines, toll):
        ...
        for i in range(0, min(cell0PointIds.GetNumberOfIds(), cell1PointIds.GetNumberOfIds())):
            ...
            if (distanceBetweenPoints > toll):
                ...
                divergingPointMISR = centerlines.GetPointData().GetArray(radiusArrayName).GetTuple1(
                    cell1PointIds.GetId(i))
                break
    ...
    def CreateParentArteryPatches(parentCenterlines, clipPoints):
        ...
        for j in range(numberOfDaughterPatches):
            ...
            for i in range(startId, cell.GetNumberOfPoints()):
                ...
                radiusArray.SetTuple1(count, parentCenterlines.GetPointData().GetArray(radiusArrayName).GetTuple1(
                    cell.GetPointId(i)))
                count += 1
    ...
    def CreateAneurysmSlice(parentCenterlines, clipPoints):
        numberOfDaughterPatches = parentCenterlines.GetNumberOfCells()
        ...
        clipIds, numberOfPatchedCenterlinesPoints = ExtractPatchesIds(parentCenterlines, clipPoints)
        print('Aneurism Clipping Point Ids ', clipIds)
        ...
        cell = vtk.vtkGenericCell()
        parentCenterlines.GetCell(0, cell)
        ...
        patchedCenterlinesCellArray.InsertNextCell(patchNumberOfPoints)
        radiusArray.SetNumberOfTuples(patchNumberOfPoints)
        print('startid:', startId, 'numberofcellpoints', patchNumberOfPoints)
        ...
        for i in range(startId, cell.GetNumberOfPoints()):
            ...
            patchedCenterlinesPoints.InsertNextPoint(point)
            patchedCenterlinesCellArray.InsertCellPoint(count)
            radiusArray.SetTuple1(count, parentCenterlines.GetPointData().GetArray(radiusArrayName).GetTuple1(i))
            count += 1
    ...
    def InterpolatePatchCenterlines(patchCenterlines, parentCenterlines):
        ...
        for i in range(parentCenterlines.GetNumberOfCells()):
            ...
            splinePoints = InterpolateTwoCells(startingCell, endingCell, numberOfInterpolationPoints,
    ...
    divergingRatioToSpacingTolerance = 1.0  # commonly 2.0; the DivergingTolerange for the identification of diverging points on each couple of centerlines is computed as (centerlineSpacing / DivergingRatioToSpacingTolerance)

    # outputfilenames:
    divergingPointsFilename = os.path.join(directory, filename + '_' + str(aneur_no) + '_divergingpoints.vtp')
    patchParentArteryFilename = os.path.join(directory, filename + '_' + str(aneur_no) + '_patchcl.vtp')
    interpolatedCenterlinesFilename = os.path.join(directory, filename + '_' + str(aneur_no) + '_interpolatedcl.vtp')
    aneurysmCenterlineFilename = os.path.join(directory, filename + '_' + str(aneur_no) + '_aneurysmcl.vtp')

    if (aneurysmType == 'lateral'):

        if (setClippingPoints == 0):
            forwardCenterlinesFilename = os.path.join(directory, filename + '_' + str(aneur_no) + '_forwardcl.vtp')
            backwardCenterlinesFilename = os.path.join(directory, filename + '_' + str(aneur_no) + '_backwardcl.vtp')
            clippingPointsFilename = os.path.join(directory, filename + '_' + str(aneur_no) + '_clippingpoints.vtp')
            print(os.path.join(directory, filename + '_' + str(aneur_no) + '_clippingpoints.vtp'))
            ...
            centerlineSpacing = math.sqrt(vtk.vtkMath.Distance2BetweenPoints(parentArteryCenterlines.GetPoint(10),
                                                                             parentArteryCenterlines.GetPoint(11)))
            ...
            upstreamClippingPoint, upstreamDivergingPoint = FindClippingPointOnParentArtery(forwardCenterlines,
                                                                                            parentArteryCenterlines,
                                                                                            divergingTolerance)
            clippingPoints.InsertNextPoint(upstreamClippingPoint)
            divergingPoints.InsertNextPoint(upstreamDivergingPoint)
            downstreamClippingPoint, downstreamDivergingPoint = FindClippingPointOnParentArtery(backwardCenterlines,
                                                                                                parentArteryCenterlines,
                                                                                                divergingTolerance)
            ...
            aneurysmCenterlines = SaveAneurysm(forwardCenterlines)


        elif (setClippingPoints == 1):
            print('Clipping Points and Parent Vessel Centerlines provided')
            parentArteryFileName = os.path.join(directory, filename + '_' + str(aneur_no) + '_parentvessel.vtp')
            clippingPointsFileName = os.path.join(directory, filename + '_' + str(aneur_no) + '_clippingpoints.vtp')
        ...
        aneurysmCl = CreateAneurysmSlice(aneurysmCenterlines, clippingPoints)
        WritePolyData(aneurysmCl, aneurysmCenterlineFilename)
    ...
    if (aneurysmType == 'terminal'):
        ...
        if (setClippingPoints == 0):
            ...
            commonClippingPoint, commonDivergingPoint = FindClippingPointOnParentArtery(parentArteryCenterlines,
                                                                                        parentArteryCenterlines,
                                                                                        divergingTolerance)
            clippingPoints.InsertNextPoint(commonClippingPoint)
            divergingPoints.InsertNextPoint(commonDivergingPoint)

            dau1ClippingPoint, dau1DivergingPoint = FindClippingPointOnParentArtery(daughter1Centerlines,
                                                                                    parentArteryCenterlines,
                                                                                    divergingTolerance)
            clippingPoints.InsertNextPoint(dau1ClippingPoint)
            divergingPoints.InsertNextPoint(dau1DivergingPoint)

            dau2ClippingPoint, dau2DivergingPoint = FindClippingPointOnParentArtery(daughter2Centerlines,
                                                                                    parentArteryCenterlines,
                                                                                    divergingTolerance)
